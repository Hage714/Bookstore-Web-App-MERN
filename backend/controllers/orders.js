...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        const orderItems = items.map(item => {
            return { book: item.id, order: order._id, quantity: item.quantity, unit_price: item.price };
        });

        const purchasedbooks = items.map(item => {
            return { book: item.id, customer: customer._id, quantity: item.quantity, price: item.price };
        })  

        const newOrderItems = await OrderItem.insertMany(orderItems)
        const newPurchasedbooks = await Purchasedbook.insertMany(purchasedbooks)
